import { getCreativeCodeWorks, getCreativeCodeWorkBySlug } from "@/data/creative";
    ...
    <div className="bg-background min-h-screen text-foreground selection:bg-brand-purple selection:text-white">
      ...
      <section className="relative min-h-[70vh] flex flex-col items-center justify-center pt-32 pb-16 overflow-hidden bg-brand-purple/5">
        <div className="absolute top-24 left-6 md:left-12 z-30">
          <TransitionLink href="/creative/creative-code" className="inline-flex items-center gap-2 text-white/70 hover:text-brand-purple transition-colors font-bold tracking-widest text-xs uppercase group bg-surface/50 backdrop-blur-md px-4 py-2 rounded-full border border-white/10">
            <ArrowLeft size={14} className="transition-transform group-hover:-translate-x-1" />
            BACK TO CREATIVE CODE
          </TransitionLink>
        </div>
        ...
        {/* Vertical Video Player Display */}
        <div className="relative z-20 w-full max-w-md mx-auto px-4 sm:px-6 flex items-center justify-center">
          <div className="w-full max-w-[340px] sm:max-w-[380px] rounded-2xl border-4 md:border-8 border-surface bg-surface shadow-2xl overflow-hidden p-2 sm:p-4">
            <div
        ...
        <div className="max-w-4xl mx-auto space-y-20">
          ...
          <div>
            <h2 className="text-xs font-mono font-bold tracking-widest text-brand-purple-light uppercase mb-6 flex items-center gap-2">
              <Layers size={14} />
              <span>02 // Computational Study</span>
            </h2>
          ...
          {/* Interaction & Algorithmic Breakdown */}
          <div className="bg-surface/40 border border-white/10 rounded-3xl p-8 md:p-12">
            <h2 className="text-xs font-mono font-bold tracking-widest text-brand-purple-light uppercase mb-8 flex items-center gap-2">
              <Sliders size={14} />
              <span>03 // System Mechanics</span>
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div>
                <h3 className="text-xs font-mono text-white/50 uppercase mb-2">Algorithmic Behavior</h3>
                <p className="text-sm text-zinc-300 font-light leading-relaxed">
                  {work.visualDetails.system}
                </p>
              </div>
              <div>
                <h3 className="text-xs font-mono text-white/50 uppercase mb-2">Interaction Loops</h3>
                <p className="text-sm text-zinc-300 font-light leading-relaxed">
                  {work.visualDetails.interaction}
                </p>
              </div>
              <div>
                <h3 className="text-xs font-mono text-white/50 uppercase mb-2">Visual Aesthetic</h3>
                <p className="text-sm text-zinc-300 font-light leading-relaxed">
                  {work.visualDetails.aesthetic}
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="py-16 md:py-24 px-6 md:px-12 bg-surface/40 border-t border-white/10 relative z-30">
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-6">
          ...
          >
            <ArrowLeft size={16} />
            <span>Return to Creative Code</span>
          </TransitionLink>
